import type { LogEntry, LogFormat, EndpointMetrics, EndpointStatus, AnalysisResult, CostEstimate } from '@/types';

// Nginx log format parser
// Example: 127.0.0.1 - - [24/Oct/2023:13:45:30 +0000] "GET /api/users HTTP/1.1" 200 1234 "-" "Mozilla/5.0"
const NGINX_REGEX = /^(?<ip>[\d.]+)\s+-\s+(?<user>\S+)\s+\[(?<timestamp>[^\]]+)\]\s+"(?<method>\S+)\s+(?<endpoint>\S+)\s+[^"]*"\s+(?<status>\d+)\s+(?<bytes>\d+).*$/;

// AWS CloudWatch/ALB log format parser
// Example: https 2023-10-24T13:45:30.123456Z app/my-loadbalancer/50dc6c495c0c9188 192.168.131.39:2817 10.0.0.50:80 0.001 0.002 0.000 200 200 34 366 "GET https://example.com:443/api/users HTTP/1.1"
const CLOUDWATCH_REGEX = /^(?<type>\S+)\s+(?<timestamp>\S+)\s+\S+\s+(?<client>\S+)\s+\S+\s+\S+\s+\S+\s+\S+\s+(?<status>\d+)\s+\d+\s+\d+\s+\d+\s+"(?<method>\S+)\s+(?<endpoint>\S+)\s+[^"]*".*$/;

// Express/morgan log format parser
// Example: ::1 - - [24/Oct/2023:13:45:30 +0000] "GET /api/users HTTP/1.1" 200 1234 "-" "Mozilla/5.0" 23.456 ms
const EXPRESS_REGEX = /^(?<ip>\S+)\s+-\s+(?<user>\S+)\s+\[(?<timestamp>[^\]]+)\]\s+"(?<method>\S+)\s+(?<endpoint>\S+)\s+[^"]*"\s+(?<status>\d+)\s+(?<bytes>\d+).*?(?<responseTime>[\d.]+)\s*ms$/;

// JSON log parser (structured logs)
...
function parseCSVLog(line: string): LogEntry | null {
  ...
  const methodPart = parts.find(p => ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'].includes(p.toUpperCase()));
...
export function detectLogFormat(sample: string): LogFormat {
  ...
  if (sample.includes('app/') && sample.includes('https ') || sample.includes('http ')) return 'cloudwatch';
  if (sample.includes('ms') && sample.match(/\d+\.?\d*\s*ms$/)) return 'express';
  if (sample.match(/^\d+\.\d+\.\d+\.\d+/) && sample.includes('[') && sample.includes(']')) return 'nginx';
...
export function parseLogs(content: string, format: LogFormat): LogEntry[] {
  ...
  const detectedFormat = format === 'auto' ? detectLogFormat(lines.find(l => l.trim()) || '') : format;
...
export function calculateCostEstimate(
  ...
  // Estimate monthly requests (assuming logs cover ~7 days)
...
export function generateAnalysisResult(
  ...
  const startTime = timestamps.length > 0 ? new Date(Math.min(...timestamps.map(d => d.getTime()))) : new Date();
  const endTime = timestamps.length > 0 ? new Date(Math.max(...timestamps.map(d => d.getTime()))) : new Date();
...
export function exportToCSV(endpoints: EndpointMetrics[]): string {
  const headers = ['Method', 'Endpoint', 'Status', 'Total Calls', 'Last Called', 'Health Score', 'Avg Response Time (ms)'];
...
var deprecatedRoutes = []Route${JSON.stringify(routes).replace(/"/g, '`"`').replace(/:/g, ' `json:"`')}
